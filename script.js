...
function setupDashboard() {
  ...
  if (safeRole === 'ADMIN' || safeRole === 'STAFF') {
    ...
    // 📌 ถ้าเป็น MENTOR ให้แสดงหน้า mentor-view
...
async function checkInModal(dayNo, timeSlot) {
  ...
  if (note !== undefined) {
    ...
    if (res.status === 'success') Swal.fire('สำเร็จ', res.message, 'success'); else Swal.fire('แจ้งเตือน', res.message, 'warning');
  }
}
...
function renderMentorPaginatedTable() {
  ...
  paginatedItems.forEach(log => {
    tbody.innerHTML += `<tr><td class="ps-3"><code>${log.personal_id}</code></td><td>${log.name}</td><td>วันที่ ${log.day_no}</td><td><span class="badge bg-secondary">${timeTranslates[log.time_slot] || log.time_slot}</span></td><td class="small">${log.timestamp}</td></tr>`;
  });
...
// ----------------------------------------
// ตารางความก้าวหน้า Admin (Pagination)
// ----------------------------------------
