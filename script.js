...
// แก้ไขใช้ createElement ป้องกันบั๊ก innerHTML
...
// -- 2. ตารางตั้งค่าข้อสอบ (Exam Config) --
async function loadExamConfigToUI() {
  ...
  if (res.status === 'success') {
    ...
    const formatDT = (dtStr) => {
      if(!dtStr) return ""; let d = new Date(dtStr);
...
async function saveSpeakerConfigFromUI() {
  ...
  for(let tr of rows) {
    ...
    newConfig.push([
      tr.querySelector('.spk-id').value, 
      tr.querySelector('.spk-name').value,
      tr.querySelector('.spk-topic').value, 
      revertDT(tr.querySelector('.spk-start').value),
      revertDT(tr.querySelector('.spk-end').value),
      tr.querySelector('.spk-active').checked ? 'TRUE' : 'FALSE'
    ]);
  }
